 ...
 * Cycle 6：#33 具名 testid 缺失即红，不得 skip。查看全部须 stale=1+site_id+month；
 * 该行 /period/{id}/calculate；?id= 抽屉 = FAIL；禁止 auto=1。金标不改。
 * trial-binding-segments 无数字 = FAIL（不得 WARN 过）。
 * 旧 import/stale spec 已改「完成」语义（夹具无方案骑手不得纯绿完成）。
 * 叠 PR #18+#19+#20+#21+#23+#24+#26+#27+#28+#31：Cycle 1/2/3/4/5 CDP。Cycle 6 叠本文件。
...
export const CONFIG = {
  ...
  // Vite app namespace 常为 undefined → 存储键 undefined-5.7.0-dev-core-access
  accessKey:
...
function sessionUuidFromToken(token, fallback = null) {
  try {
    const payload = JSON.parse(
      Buffer.from(String(token).split('.')[1], 'base64url').toString('utf8'),
    );
    return payload.session_uuid || fallback;
  } catch {
    return fallback;
  }
}

async function fetchAccessCodes(token, apiUrl = CONFIG.apiUrl) {
  const res = await fetch(`${apiUrl}/api/v1/auth/codes`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!res.ok) {
    throw new Error(`GET /auth/codes failed: ${res.status}`);
  }
  const json = await res.json();
  const codes = json?.data;
  if (!Array.isArray(codes)) {
    throw new Error(`GET /auth/codes 未返回权限码列表：${JSON.stringify(json).slice(0, 200)}`);
  }
  return codes;
}
...
export async function injectAdmin(page, token, sessionUuid = null) {
  ...
  const resolvedSession = sessionUuidFromToken(token, sessionUuid);
  const accessCodes = await fetchAccessCodes(token);
  await page.evaluate(
    ({ keys, token, sessionUuid, accessCodes }) => {
      localStorage.clear();
      const payload = {
        accessToken: token,
    ...
    { keys, token, sessionUuid: resolvedSession, accessCodes },
  );
}
...
export async function connectBrowser() {
  ...
  // 每场景独立 page，避免 keep-alive 把上一角色菜单/抽屉留在 DOM
